...
class BedrockKnowledgeBase:
    ...
    def _validate_models(self):
        ...
        if self.multi_modal:
            multimodal_compatible_models = [
                "cohere.embed-v4:0", "us.cohere.embed-v4:0", "global.cohere.embed-v4:0",
                "amazon.titan-embed-image-v1"
            ]
            if self.embedding_model not in multimodal_compatible_models:
                raise ValueError(f"Multimodal mode requires a multimodal-compatible embedding model. "
    ...
    def _get_embedding_dimensions(self):
        ...
        if base_dimensions is None:
            raise ValueError(f"Dimensions not defined for embedding model: {self.embedding_model}")
        
        # For multimodal usage with Cohere v4, ensure we use the correct dimensions
        if self.multi_modal and self.embedding_model in ["cohere.embed-v4:0", "us.cohere.embed-v4:0", "global.cohere.embed-v4:0"]:
    ...
    def _get_model_arn(self, model_id):
        ...
        inference_profile_prefixes = ("us.", "us-gov.", "eu.", "apac.", "au.", "ca.", "global.")
    ...
    def create_s3_bucket(self, multi_modal=False):
        ...
        for bucket_name in buckets_to_check:
            try:
                ...
                print(f'Bucket {bucket_name} already exists and you have access - reusing it!')
                ...
                if error_code == 'NoSuchBucket':
                    ...
                    print(f'Bucket {bucket_name} exists but you do not have access - will try to create with different name')
                else:
                    print(f'Could not access bucket {bucket_name}: {error_code} - will try to create it')
        ...
        for bucket_name in buckets_to_create:
            ...
            try:
                ...
                if error_code == 'BucketAlreadyExists':
                    print(f'Bucket {bucket_name} already exists globally and is owned by another account.')
                    print(f'Please choose a different bucket name or use an existing bucket you own.')
                    raise ValueError(f"Bucket name '{bucket_name}' is already taken globally. Please use a different name.")
                elif error_code == 'BucketAlreadyOwnedByYou':
                    print(f'Bucket {bucket_name} already exists and is owned by you - this should have been caught earlier.')
                    # This shouldn't happen due to our head_bucket check, but handle it gracefully
                    existing_buckets.append(bucket_name)
                elif error_code == 'AccessDenied':
                    print(f'Access denied when creating bucket {bucket_name}. This could be due to:')
    ...
    def configure_log_delivery(self, knowledge_base_arn):
        ...
        try:
            ...
            try:
                ...
                create_source_response = {'deliverySource': sources['deliverySources'][0]}
            ...
            try:
                ...
                print(f"Delivery destination {delivery_destination_name} already exists")
                ...
                create_dest_response = {'deliveryDestination': destinations['deliveryDestinations'][0]}
            ...
            try:
                create_delivery_response = self.logs_client.create_delivery(
                    ...
                    deliveryDestinationArn=create_dest_response['deliveryDestination']['arn'],
                    tags={
                        'Purpose': 'BedrockKnowledgeBase',
                        'KnowledgeBaseName': self.kb_name
                    }
                ...
                print(f"Log delivery status: {create_delivery_response['delivery']['deliveryStatus']}")
            except self.logs_client.exceptions.ResourceAlreadyExistsException:
            ...
            print("Knowledge base will work without logging, but ingestion logs won't be available")
    ...
    def create_lambda_role(self):
        ...
        try:
            ...
            if "EntityAlreadyExists" in str(e) or "already exists" in str(e):
                print(f"Lambda IAM role {lambda_function_role} already exists, retrieving it...")
        ...
        try:
            ...
            if "EntityAlreadyExists" in str(e) or "already exists" in str(e):
                print(f"S3 access policy {s3_access_policy_name} already exists, retrieving it...")
                policy_arn = f"arn:aws:iam::{self.account_number}:policy/{s3_access_policy_name}"
            else:
    ...
    def create_bedrock_execution_role_multi_ds(self, bucket_names=None, secrets_arns=None):
        ...
        try:
            ...
            if "EntityAlreadyExists" in str(e) or "already exists" in str(e):
                print(f"IAM role {self.kb_execution_role_name} already exists, retrieving it...")
        ...
        for policy_name, policy_document, description in policies:
            ...
            try:
                ...
                print(f"Attached policy {policy_name} to role {bedrock_kb_execution_role['Role']['RoleName']}")
            except Exception as e:
    ...
    def create_vector_index(self):
        ...
        try:
            ...
            print(f'Embedding model: {self.embedding_model}, Dimensions: {dimensions}, Multimodal: {self.multi_modal}')
            raise
            ...
            if self.multi_modal:
                if self.parser == "BEDROCK_FOUNDATION_MODEL":
                    parsing_configuration = {"bedrockFoundationModelConfiguration": 
                                             {"parsingModality": "MULTIMODAL", "modelArn": self._get_model_arn(self.generation_model)}, 
    ...
    def delete_cloudwatch_log_group(self):
        ...
        try:
            ...
            print(f"======== CloudWatch Log Group {self.log_group_name} deleted =========")
        except self.logs_client.exceptions.ResourceNotFoundException:
            print(f"CloudWatch Log Group {self.log_group_name} not found, skipping deletion")
        except Exception as e:
            print(f"Error deleting CloudWatch Log Group {self.log_group_name}: {str(e)}")
